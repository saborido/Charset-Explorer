;===> Charset Explorer pre RC2 <===> 'char2grid.asm' v3 (569 bytes en total (con INCLUDEs y CON AGUSTINA la subrutina) <===
...
;Tokens A (0-31): Control Codes ("." blue).
    ld a, (hl)
    cp 32
    jr nc, C2G_TOKEN
...
;Tokens A2 (32-127): Characters (CHR$ green).
C2G_TOKEN:
    ld a, (hl)
    cp 128
    jr nc, C2G_TOKEN_B
...
;Tokens B (128-143):  Characters - Graphics Blocks (CHR$ red).
C2G_TOKEN_B:
    ld a, (hl)
    cp 144
    jr nc, C2G_TOKEN_C
...
CHR_LD
	...
	call z, DRW_DOT             ;Si es un 0, dibujamos el dot.
...
DRW_DOT:
;Configuramos el color de los píxeles OFF (dots):
    ld a, 16
    rst 16
    ld a, (DOTCOL_)             ;Paper dot color + 1.
    dec a                       ;Restamos 1 para obtener el color correcto.
    rst 16

	call DRW_INI
    jr nz, JMP_BIN2             ;Salto a JMP_BIN a imprimir ceros.

;Modo normal, imprime pixeles definidos con PIX_CHR:

    ld a, "."					;Imprimimos el dot
    rst 16                      ;(el modo Bin está OFF).
jr X_INC                    ;Saltamos el modo Bin y volvemos a la rutina principal.

;Modo Binario, imprime ceros:

JMP_BIN2:
    ld a, "0"
    rst 16

jr X_INC					;incrementamos el eje X y volvemos a la rutina principal.
